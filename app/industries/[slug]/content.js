...
export default function FacultyManagement({ industry }) {
  ...
  const defaultFaqs = [
    {
      ...
      answer: "Customer relationship management for the automotive industry is a system for managing all of your company's interactions with current and potential customers, as well as critical information like driver, vehicle, retail, and automotive financial data."
    },
    {
      question: "How do I choose the best automotive CRM for my business?",
      answer: "First, determine what you want an automotive CRM to solve. An automotive CRM should include functionality, such as vehicle and driver information, automotive lead management, partner performance management, inventory management, fleet management, embedded analytics and AI, and more."
    },
    {
      question: "What are the benefits of using a CRM for automotive?",
      answer: "An automotive CRM can help you get 360-degree visibility into your customers and their households and vehicles, build a robust sales pipeline, intelligently manage your vehicle and parts product portfolio, and deliver meaningful service experiences for drivers and households."
    },
    {
      question: "Does my business need an automotive CRM?",
      answer: "Consider an automotive CRM if you find that your customer-facing interactions are misguided, that you're missing upsell opportunities, or if you want to better serve drivers and their households. An automotive CRM like Automotive Cloud helps teams take action fast and delight every customer."
    }
  ];
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      ...
      try {
        ...
        setFaqs(response.data.faqs.filter((item) => item?.relatedIndustries?._id === industry?._id));
      } catch (error) {
  ...
  const faqsToRender = faqs.length > 0 && faqs[0]?.questions?.length > 0
    ? faqs[0].questions
    : defaultFaqs
...
const FaqItem = ({ question, answer, isOpen, toggleOpen }) => {
  return (
    <div className={`w-full border-b-2 border-[#446e6d44]`} onClick={toggleOpen}>
      ...
      <p className={`${isOpen ? 'block' : 'hidden'} mb-6`}>
        {answer}
      </p>
    </div>
  );
