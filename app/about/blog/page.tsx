  ...
  useEffect(()=>{
    const fetchdata = async () => {
      try {
        ...
        setCategories(res?.data?.blogs?.map((item: any) => item.type));
        ...
        <div className='w-[1320px] mx-auto flex items-start z-20 gap-7'>
            ...
            <div className='w-full mb-6 p-6 bg-[#F5F5F5] text-[#0a1121]' style={{
              ...
              blogData.filter((item)=>{
                ...
                return(
                  <div className='w-full mb-5  flex  hover:text-[#37c0bd] transition-colors duration-700 cursor-pointer gap-3' key={index}>
                ...
                <span className='text-[12px] font-medium text-gray-500 flex items-center gap-1'><CalendarMonthIcon fontSize='inherit'/> {formatDate(item?.createdAt)}</span>
                </div>
              </div>
                )
              })
