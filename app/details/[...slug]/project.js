...
export default function Page({ project }) {
  const [openSections, setOpenSections] = useState(
    project?.section?.map(() => false) || [] // Initialize all sections as closed
  );
  ...
  const toggleSection = (index) => {
    setOpenSections((prev) =>
      prev.map((isOpen, i) => (i === index ? !isOpen : false)) // Toggle the clicked section, close others
    );
  };
  ...
  return (
    ...
    <title>{project?.Title ? `Webmedigital - ${project?.Title}` : 'Webmedigital - Projects'}</title>
      <Head>
        ...
        <meta property="og:image" content={project?.image || '/default-image.jpg'} />
      ...
      <header className="flex min-h-screen">
        ...
        <section className="basis-1/2 min-h-full pt-16 flex flex-col">
          <div className="pt-[10%]">
            <img src={project?.image} alt="Project overview" />
          </div>
        </section>
      </header>
      ...
      {project?.section?.map((item, index) => (
        <section key={index} className={`${index % 2 === 0 ? 'my-24' : ''}`}>
          <div className="w-4/5 mx-auto flex justify-between gap-[15%]">
            ...
            >
              <h2 className="text-5xl font-semibold text-[#446E6D]">{item.Heading}</h2>
                ...
                {Array.from(Array(3)).map((_, subIndex) => (
                  <div
                    ...
                    className={`w-full border-l-4 pl-6 cursor-pointer ${
                      openSections[index] === subIndex ? 'border-l-[#446E6D]' : 'border-l-white'
                    }`}
                    onClick={() =>
                      setOpenSections((prev) =>
                        prev.map((isOpen, i) =>
                          i === index ? (isOpen === subIndex ? false : subIndex) : isOpen
                        )
                      )
                    }
                  >
                    ...
                    >
                      {item[`subHeadingdetails${subIndex + 1}`]}
                    </p>
                  </div>
                ))}
              </div>
