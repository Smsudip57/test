...
export default function Page({ details }) {
  const [openSections, setOpenSections] = useState(
    details?.Service?.section?.map(() => false) || [] // Initialize all sections as closed
  );
  ...
  <section key={index} className={`${index % 2 === 0 ? 'my-24' : ''}`}>
    <div className="w-4/5 mx-auto flex justify-between gap-[15%]">
      ...
      >
        <h2 className="text-5xl font-semibold text-[#446E6D]">{item.Title}</h2>
        <div className="text-xl font-sans mt-12 border-l-2 border-[#446E6D] flex flex-col gap-8">
          {Array.from(Array(3)).map((_, subIndex) => (
            <div
              ...
              onClick={() =>
                setOpenSections((prev) => {
                  ...
                  updatedSections[index] = updatedSections[index] === subIndex ? null : subIndex;
            ...
            >
              ...
              >
                {item[`subHeading${subIndex + 1}edtails`]} {/* Match your schema here */}
              </p>
            </div>
          ))}
        </div>
