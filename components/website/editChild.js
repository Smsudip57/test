...
export default function EditProductList() {
  ...
  // Form state for editing - Updated to include itemsTag as simple string
  ...
  const fetchServicesData = async () => {
    try {
      setLoading(true);
      const childResponse = await axios.get("/api/child/get", {
        withCredentials: true,
      });
      const parentResponse = await axios.get("/api/product/get", {
        withCredentials: true,
      });
  ...
  // Open edit mode with pre-filled values - Updated to include itemsTag
  const handleEditClick = useCallback((childService) => {
    ...
    setChildServiceData({
      ...
      itemsTag: childService.itemsTag || "", // Simple string field
  ...
  const validateForm = useCallback(() => {
    ...
    if (!childServiceData.itemsTag || !childServiceData.itemsTag.trim()) {
  ...
  // Cancel edit mode - Reset to simple state
  ...
  const handleSave = useCallback(() => {
    const saveChildService = async () => {
      ...
      try {
        ...
        formData.append("itemsTag", childServiceData.itemsTag); // Simple string field
                    ...
                    <div className="space-y-5">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          name="itemsTag"
                          type="text"
                          value={childServiceData.itemsTag}
                          onChange={handleInputChange}
                          className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#446E6D] focus:border-[#446E6D]"
