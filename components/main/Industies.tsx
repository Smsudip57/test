...
const Projects = () => {
  ...
  <div className="mx-auto mb-10 sm:mb-14 px-4 max-w-6xl text-center">
    ...
    <h1 className="font-lora text-4xl text-green-900 font-bold mb-6">INDUSTRIES</h1>
    <p className="text-xl text-center mb-4 ">Your Industry, Our Expertise: Customized IT Solutions for Every Sector.</p>
  </div>
  <div className="flex flex-col items-center py-0 mx-auto max-w-[1200px]">
      ...
      <div className="flex flex-wrap gap-4 justify-center items-center place-items-center">
        {industries.map((name, index) => (
          <a href={`/industries/${name.toLowerCase().split(" ").join('-')}`} className="basis-[14.28%] flex justify-center items-center  shadow-lg border rounded-lg p-4"  key={index}>
          <div className="">
